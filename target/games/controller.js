    ...
    async updateGame(id, update) {
        ...
        if (update.color) {
            if (!colors.includes(update.color))
                throw new routing_controllers_1.BadRequestError('Please choose among these colors: red, blue, green, yellow or magenta');
        }
        if (update.board) {
            if (moves(game.board, update.board) > 1)
                throw new routing_controllers_1.BadRequestError('only one move per turn is allowed');
        }
